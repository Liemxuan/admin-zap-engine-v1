...
export const CategoryList: React.FC = () => {
    ...
    const { categories, total, loading: apiLoading, params, handleSearch, handlePageChange } = useCategories({ pageIndex: 1, pageSize: 10 });
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-slate-200 bg-slate-50">
                                <th className="px-4 py-3 text-left text-sm font-semibold text-slate-600 w-12 whitespace-nowrap">{t('category.id')}</th>
                                <th className="px-4 py-3 text-left text-sm font-semibold text-slate-600 whitespace-nowrap">{t('category.name')}</th>
                                <th className="px-4 py-3 text-right text-sm font-semibold text-slate-600 whitespace-nowrap">{t('category.itemCount')}</th>
                                <th className="px-4 py-3 text-left text-sm font-semibold text-slate-600 whitespace-nowrap">{t('category.status')}</th>
                            ...
                            {loading ? (
                                Array.from({ length: 5 }).map((_, i) => (
                                    <tr key={i} className="animate-pulse">
                                        ...
                                        <td className="px-4 py-3"><div className="h-3 bg-slate-200 rounded w-8 ml-auto" /></td>
                                ...
                                categories.map((item) => (
                                    <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="px-4 py-3 text-right text-xs text-slate-600">{item.productCount}</td>
                                        <td className="px-4 py-3">
                                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${
                                                item.status === 'active' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'
                                            }`}>
                                                {item.status === 'active' ? t('category.active') : t('category.inactive')}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3">
                                            <div className="flex items-center justify-center gap-1">
                                                <button onClick={() => navigate(`/${language}/category/${item.id}/edit`)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors" title={t('common.edit')}>
                                ...
                                <tr>
                                    <td colSpan={6} className="px-4 py-12 text-center">
                                        ...
                                        <p className="text-sm font-medium text-slate-500">{t('category.notFound')}</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
